"""core_gap_audit.py Ã¢â‚¬â€ CORE-wide manual work detector.
...
from core_config import _env_int, _sbh_count_svc, sb_get
    ...
    if gaps:
        ...
        for gap in gaps[:8]:
            lines.append(
                ...
                f"{str(gap.get('title') or '')[:120]} Ã¢â‚¬â€ {_escape(str(gap.get('manual_action') or ''), 180)}"
            )
    else:
        lines.append("")
