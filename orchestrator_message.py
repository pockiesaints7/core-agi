"""
Standardized message object for CORE AGI orchestrator.
All layers receive and pass this single object.
"""
...
class OrchestratorMessage:
    ...
    intent: Optional[str] = None  # L3 Intent classification
    plan: Dict[str, Any] = field(default_factory=dict)  # L4 Reasoning plan
    ...
    evolutions_proposed: List[Dict[str, Any]] = field(default_factory=list)  # L7 Refinement
    ...
    def track_layer(self, layer: str):
        """Track layer execution for debugging."""
        self.layer_stack.append(f"{layer}@{datetime.now().timestamp()}")
