...
TOOLS = {
    ...
    "debug_fn":               {"fn": t_debug_fn,               "perm": "READ",    "args": ["fn_name", "dry_run", "extra_args"],
                               "desc": "Battle-tested debug harness for any CORE function. Runs fn_name through staged execution: 0_resolve (find function), 1_preflight (data fetches), 2_execute (run with optional write intercept), 4_write_intercepted (if dry_run=True). Returns per-stage status+data+error+trace. Use to diagnose silent failures without touching production data. fn_name=any function in core_train/core_tools/core_config/core_github. dry_run=true by default."},
    ...
    "debug_fn":               {"fn": t_debug_fn,               "perm": "READ",    "args": ["fn_name", "dry_run", "extra_args"],
                               "desc": "Battle-tested debug harness for any CORE function. fn_name=name of function to debug (e.g. '_run_simulation_batch'). dry_run=true (default) runs all stages EXCEPT the final DB write -- safe for production. Returns staged breakdown: 0_resolve, 1_preflight, 2_execute, 4_write_intercepted. First failed stage tells you exactly where it broke. Use before declaring any function 'dead'."},
