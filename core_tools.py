    ...
    try:
        ...
        if existing:
            ...
            if ex_instr and new_instr and ex_instr != new_instr:
                ...
                return {"ok": False, "conflict": True, "conflict_type": "instruction", "action": "blocked",
                        "existing_instruction": ex.get("instruction","")[:200], "existing_content": ex_content[:200],
            ...
            # Check 3: content conflict (substantial differing content)
            if ex_content and new_content and ex_content.lower() != new_content.lower():
                if len(ex_content) > 50 and len(new_content) > 50:
                    ...
                    return {"ok": False, "conflict": True, "conflict_type": "content", "action": "blocked",
                            "existing_instruction": ex.get("instruction","")[:200], "existing_content": ex_content[:200],
...
TOOLS = {
    ...
    "session_end":            {"fn": t_session_end,            "perm": "WRITE",   "args": ["summary", "actions", "domain", "patterns", "quality", "skill_file_updated", "force_close", "active_task_ids"],
                               "desc": "One-call session close. actions=pipe-separated strings (| only, NOT comma). quality clamped 0.0-1.0 auto. BEFORE calling: (1) log_mistake for every error, (2) add_knowledge for every new insight, (3) changelog_add for every deploy, (4) update task statuses in task_queue. active_task_ids=pipe-separated UUIDs of tasks touched -- warns if any still pending/in_progress. TASK-21.B gate: if patterns non-empty, query KB domain=core_agi.identity topic=active_skill_filename for current path, write rules to that file via Desktop Commander:edit_block, then pass skill_file_updated=true. HARD RULE: force_close=true is OWNER-INVOKED ONLY -- CORE must NEVER self-authorize force_close to bypass the skill_file gate. Every force_close use is audit-logged to knowledge_base. Returns: training_ok (bool), duration_seconds, reflection_warning if hot reflection failed, task_status_warnings if tasks left open."},
