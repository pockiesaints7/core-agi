...
def t_session_end(summary: str, actions: str, domain: str = "general",
    ...
    completed_tasks: pipe-separated task IDs/subtasks to tick in SESSION.md e.g. '7.1|7.2|7.3'
    new_step: if set, updates the '## Current Step' line in SESSION.md
    ...
    try:
        ...
        # 3. Update SESSION.md — tick completed tasks + append session log entry
        ...
        try:
            ...
            if completed_tasks.strip():
                for task_id in completed_tasks.split("|"):
                    ...
                    # Also try matching just the number mid-line
            ...
            # Find the session log table and append after header separator
            if "## 7. SESSION LOG" in content or "## SESSION LOG" in content:
                # Insert after the header row of the table
                lines = content.splitlines()
                for i, line in enumerate(lines):
                    if line.startswith("| Date |") or line.startswith("| date |"):
                        # Next line should be separator, insert after it
                        sep_line = i + 1
            ...
            if content != original:
                gh_write("SESSION.md", content,
                         f"chore(session): auto-update SESSION.md — {date_str} session close")
...
TOOLS = {
    ...
    "session_end":            {"fn": t_session_end,            "perm": "WRITE",   "args": ["summary", "actions", "domain", "patterns", "quality", "completed_tasks", "new_step"],
                               "desc": "One-call session close. completed_tasks=pipe-separated task IDs to tick in SESSION.md (e.g. '7.1|7.2'). new_step=update Current Step. Always logs session + Groq hot_reflection."},
